...
<body>
	...
	<form:form id="inputForm" modelAttribute="workPlan" action="${ctx}/work/workPlan/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
                <sys:treeselect id="worktype" name="workType.id" value="${workPlan.workType.id}" labelName="workType.name" labelValue="${workPlan.workType.name}"
					title="工作类别" url="/work/workType/treeData" allowClear="${office.currentUser.admin}" />
			</div>
		</div>
		<div class="control-group">
		...
		<div class="control-group">
			...
			<div class="controls">
				 <sys:treeselect id="personLiableId" name="personLiable.id" value="${workPlan.personLiable.id}" labelName="personLiable.name" labelValue="${workPlan.personLiable.name}"
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:checkboxes path="frequency" items="${fns:getDictList('frequency_months')}" itemLabel="label" itemValue="value"  htmlEscape="false" class=""/>
			</div>
		</div>
		<div class="control-group">
		...
		<c:if test="${not empty workPlan.id}">
			<div class="control-group">
				...
				<div class="controls">
					<form:checkbox path="isCancel" htmlEscape="false" maxlength="1" class="input-xlarge "/>
				</div>
			</div>
			<div class="control-group">
				<label class="control-label">取消原因:</label>
				<div class="controls">
					<form:textarea path="cancelReason" htmlEscape="false" rows="4" maxlength="255" class="input-xxlarge "/>
				</div>
			</div>
			<div class="control-group">
				<label class="control-label">是否删除:</label>
				<div class="controls">
					<form:checkbox path="delFlag" htmlEscape="false" maxlength="1" value="1" class="input-xlarge "/>
				</div>
			</div>
			<div class="control-group">
				<label class="control-label">删除原因:</label>
				<div class="controls">
					<form:textarea path="removeReason" htmlEscape="false" rows="4" maxlength="255" class="input-xxlarge "/>
				</div>
			</div>
		</c:if>
