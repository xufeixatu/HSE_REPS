...
<body>
	...
	<script type="text/template" id="treeTableTpl">
		<tr id="{{row.id}}" pId="{{pid}}">
			<td><a href="${ctx}/work/workPlan/exec_form?id={{row.id}}&planType=${planTypeDict.value}">
					{{row.name}}
				</a>
			</td>
			<td>
			...
			<shiro:hasPermission name="work:workPlan:edit"><td>
   				<a href="${ctx}/work/workPlan2/dept_feedback_form?id={{row.id}}&remainId={{row.remainId}}&planType=${planTypeDict.value}&noedit=true">反馈</a>
				<a href="${ctx}/work/workPlan2/dept_remain_feedback_list?id={{row.id}}&remainId={{row.remainId}}&planType=${planTypeDict.value}">【查看反馈列表】</a>
